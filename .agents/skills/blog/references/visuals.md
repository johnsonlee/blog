...
解释型配图呈现文字难以说明的关系；叙事图片可以记录真实场景、人物或物件，不必伪装成模型升级。正文主线稳定后按需要选图，不复制上一篇的图型组合。封面属于发布资产，适用范围见 [publishing.md](publishing.md)，不受“没有必要就不画解释图”的判断影响。
...
解释图若没有增加关系、比较或推理所需的信息，就不画。叙事图片核对它与经历的关系、真实性和来源；不为补现场感生成冒充真实经历的照片。封面按下文独立设计。
...
- 解释图出现在读者需要理解对应关系的位置；叙事图片放在相关场景。不要在每章末尾重复图解摘要。
...
新建正式文章、草稿转正式文章及完整发布准备时，中文标题和主视觉稳定后生成 5:2 横版封面，保存到 `../wxmp/<slug>.png`。Hexo 自动 OG card 不能替代。用户提供样例时必须作为 reference image。局部改稿按 `publishing.md` 检查受影响的既有资产，不自动生成整套新封面。
...
SVG 源码通过 XML 检查不代表图正确。对本次新增或修改的视觉资产实际渲染，按资产类型检查适用项目；有双语版本的两版都检查：
...
发现一类几何问题时检查受影响整组图和双语版本。最终核对解释图是否完成 figure brief 的目的、叙事图是否对应原始材料、封面是否符合标题与裁切要求。
