...
苹果给开发者的信号同样清楚。App Intents schemas 可以把 App 的实体放进 Spotlight semantic index，把动作暴露给 Siri；View Annotations 则让系统理解屏幕上正在显示的对象。
...
Microsoft 和 NVIDIA 从 PC 出发。它们强调本地 Agent、统一内存和持续运行的工作负载，NVIDIA 甚至直接把 agents 称为 personal computing 的未来。
