...
用 Claude Code 写了几个月代码，发现一个反直觉的事实：**它最大的问题不是不够聪明，而是太"勤快"了。** 你让它改一个文件，它在主 session 里一路干到底——读代码、分析依赖、写 patch、跑测试。整个 context window 被塞得满满当当，你想插一句"顺便看看另一个 bug"，得等它干完才行。就像跟一个能力很强的同事共用一张桌子，他把整张桌子铺满了资料，你连放杯咖啡的地方都没有。
...
**当工具有了智能，使用方式本身就是架构设计。** 那张桌子还是那张桌子，但你可以决定谁坐主位。
