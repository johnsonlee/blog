...
微软要的是 Windows 重新成为 AI 时代的生产力入口，所以它给 Copilot+ PC 定了 40 TOPS NPU 的硬门槛。这道门槛是说给整个 OEM 阵营听的：下一代 PC 的底线，已经从 CPU 跑分挪到了本地推理能力。NVIDIA 嫌这还不够重，它要把开发者、创作者、模型调试、代码生成这些重活拉到本地，做成 AI workstation。
...
三家的说法各不相同，底下却是同一件事——**把一部分推理负载从云端搬回本地。**
...
真正的变化在于：模型能力被放进了 Foundation Models framework，App 通过系统 API 去调，自己不用再各塞一个 LLM。这一步把 LLM 从"某个应用的功能"变成了"操作系统的能力"。

App 发请求，系统判断权限、挑选上下文、调用端侧模型，必要时再路由到 Private Cloud Compute，最后把结果还给 App。App 自己不持有模型，也不持有你的上下文，它只是发起方。

这才是 AI PC 时代最关键的能力分水岭：

> 设备能跑模型只是起点，操作系统把它变成默认能力才是终点。
...
这句话不够。大家盯着模型文件有多大，真正卡人的却是**运行时要占多少 DRAM**。
...
硬塞一个 20B dense 进去是死路。苹果端侧路线的关键，在于换了一套内存模型：
...
所以苹果真正要解决的是 **DRAM footprint**，"参数太多"只是表象。这也是整个 AI PC 时代绕不开的同一道题——不管 iPhone、MacBook、Windows AI PC 还是 RTX workstation，最后都卡在权重怎么放、KV Cache 怎么控、上下文怎么选、带宽够不够、功耗压不压得住。
...
正因为换专家有成本，服务器那套 token-level MoE 在手机上行不通。苹果走的是 prompt-level routing：先判断这个 prompt 需要什么能力，挑一组专家加载进 DRAM，整段生成期间尽量复用，必要时再周期性调整。

服务器可以靠 HBM 和大显存硬扛 token 级的频繁换入换出，手机和轻薄本不行。所以端侧 AI 的工程思路只有一条路——减少活跃工作集，提高缓存复用，压住内存抖动和功耗。这套思路所有人迟早都要走，苹果只是先走了一步。
...
所以未来的格局是分层的：本地模型做第一层，云端模型做第二层，OS 和 Agent 在中间做任务路由。这也是 Private Cloud Compute 的意义——端侧只是第一道过滤器：小任务本地解决，重任务上云，隐私数据尽量不出设备，必要时让用户确认。
...
谁控制了默认入口，谁就决定这个任务本地跑还是云端跑、用哪个模型、调哪个 App、拿哪些上下文、用户最后看到哪个结果。这才是 AI PC 和 WWDC 真正的共同主线：入口归谁。至于 NPU 几个 TOPS，是次要的。

而苹果在这件事上的位置很特殊。它单个模型未必最强，但手里同时握着 Apple Silicon、统一内存、端侧模型、系统权限、个人上下文、App Intents、跨设备生态和 PCC。这一整套天然适合端侧 AI，尤其是 Mac——如果 iPhone 是个人 AI 的随身入口，Mac 就是本地 AI 的生产力入口。

到那时，内存要扛的是模型权重、KV Cache、本地上下文、多模态 buffer 和 agent 工作区。"开几个 Chrome tab"那套标准，早就过时了。8GB 在过去还能糊弄普通用户，AI PC 时代会越来越尴尬。统一内存的价值，会被重新定价。

## 回来的不是那台旧 PC

AI PC 这个词最容易骗人的地方，是让人以为 PC 行业要回到那个增长的黄金年代。

不是。旧 PC 的核心是浏览器、Office、本地文件、键鼠和 CPU 性能；新 PC 的核心是本地模型、个人上下文、低延迟推理、多模态输入、agent 工作流和云端协同。真正活过来的，是个人计算设备重新拿回了不可替代的本地价值；那个叫 PC 的盒子，只是它的载体。
...
谁先把它做成默认，谁就拿走下一个十年的入口。苹果在 AI 上掉队了那么多年——这一次，它押对了边。
