...
我一开始也觉得没意思。Siri 这个入口存在太久了，换个大模型不稀奇。可这次它处理的对象变了：屏幕、个人上下文、App 动作和执行权限。Siri 开始从回答走向动手。

一旦要动手，问题就落到 iPhone 上。自然语言先在本地理解，能做的本地做，做不了的交给 PCC。听起来像产品话术，拆开就是一笔硬账：手机本地得跑得动一个足够有用的 LLM。

权重放哪，DRAM 放多少，KV Cache 怎么控，什么时候上云，这些账算不清，发布会讲得再顺也落不了地。先不聊 Siri 聪不聪明，先算 iPhone 这笔账。

<!-- more -->

WWDC26 这次把几个原本分散的东西收到了 Siri 后面：Apple Intelligence、Foundation Models、App Intents、Private Cloud Compute、Core AI。单独看都不稀奇，放在一起就不一样了。
...
把这些东西放回 Siri 后面，我才觉得这条线成立：自然语言入口、系统上下文、App action、本地模型、PCC fallback、底层 runtime，全都接到了同一条路径上。
