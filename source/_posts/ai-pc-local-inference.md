...
所以看到 WWDC26 再把 Siri 推回 Apple Intelligence 的中心，我的第一反应是：就这？这不是早就有了吗？

我一开始也觉得没意思。Siri 这个入口存在太久了，换个大模型不稀奇。可这次它处理的对象变了：屏幕、个人上下文、App 动作和执行权限。它怎么知道我现在看的是什么？怎么把一句话变成 App 里的动作？执行前的权限和确认谁来管？

这几个问题把 Siri 往前推了一步：它要动手。一旦要动手，问题就落到 iPhone 上：如果自然语言要先在本地理解，能做的本地做，做不了的再交给 PCC，那手机本地到底要承担多少推理？

我开始认真看，是因为问题落到了工程账上：iPhone 凭什么跑得动一个足够有用的 LLM？权重放哪，DRAM 放多少，KV Cache 怎么控，什么时候上云，这些账算不清，发布会讲得再顺也落不了地。

<!-- more -->

WWDC26 这次把几个原本分散的东西收到了 Siri 后面：Apple Intelligence、Foundation Models、App Intents、Private Cloud Compute、Core AI。单独看都不稀奇，放在一起就会逼出一个问题：Siri 到底还是语音入口，还是系统级 AI router？
...
把这些东西放回 Siri 后面，我才觉得这条线成立。自然语言入口、系统上下文、App action、本地模型、PCC fallback、底层 runtime，全都接到了同一条路径上。问题也跟着变尖：这条路径里，哪些必须留在本地？
...
老 Siri 卡在 fixed intent 和后端服务里。新 Siri 要同时处理端侧模型、系统上下文、App schema、runtime 和隐私边界。旧的问答框架撑不住这一套。

## Siri 接上系统推理

想象一个很普通的请求：把这张登机牌发给我老婆。

这句话放在聊天框里很好回答，放在系统里很难执行。系统要知道屏幕上哪张图是登机牌，通讯录里谁是老婆，应该用哪个消息 App，消息里要带什么附件，执行前要不要确认。过去这种需求很容易掉进固定 intent 的缝里。

App Intents 和 App Schemas 补的是这张动作图谱。App 把自己的 entity、action、schema、semantic index、onscreen context 交给系统。LLM 负责理解自然语言和上下文，App Intents 负责把理解结果落到可执行动作上。

Siri 和普通 chatbot 的差别就在这里。chatbot 生成文本，Siri 调用系统能力。生成一句漂亮回答不值钱，替用户把事情做完才值钱。

动作链路越长，本地模型越重要。每次都把个人上下文、屏幕内容、App 数据发到云端，隐私、延迟、成本都会炸。iPhone 必须先在本地完成足够多的理解和过滤。

到这一步，我就不再纠结 Siri 会不会聊天了。手机怎么跑 LLM，才是后面那层账。
...
“iPhone 跑 20B”这句话很容易听歪。更准确的说法是：iPhone 上有一个 20B 级别的参数池，单次请求只把 1B 到 4B active set 放进热路径。
...
实际运行还要加 KV Cache 和各种 buffer，但压力已经从整块 20B 变成当前 1B 到 4B。手机跑 LLM 的门，是从内存账打开的。
...
Apple 在 IFP 论文里给了一组数：9B 级别模型按输入动态剪到 3B active 后，在数学和代码等领域比 3B dense model 高 5 到 8 个百分点，效果接近 9B dense，TTFT 又接近 3B dense。

手机端需要的正是这种形态。大参数池放在冷端，当前任务只组装一个足够强的小模型。

## NAND DRAM 和 routing

把 NAND、DRAM、router 三者放到一张图里，AFM 3 Core Advanced 的账会更清楚。
...
NAND 管容量，DRAM 管热路径。router 的任务是别让 NAND 里的大模型整块进入 DRAM。
...
低 bit 能力从训练阶段开始塑形，发布前打包只是最后一步。放到 AFM 3 Core Advanced 上看，sparse 先把 20B 剪成当前 1B 到 4B，QAT 再把这部分压进手机能承受的 DRAM footprint。
...
如果我是 Apple，我不会让每个 App 自己接一个模型。那会变成权限、上下文、成本和体验的灾难。更合理的做法，是让 App 把 entity、action、schema、semantic index 交给系统，让模型在系统层理解自然语言，再由 App Intents 把理解结果落到动作上。

Foundation Models framework 管模型来源。同一套抽象下，本地 Apple Foundation Models、Private Cloud Compute、第三方 provider 都可以接进来。开发者甚至可以实现自己的 Language Model provider。模型放在哪里，谁来跑，什么时候上云，全部收回系统层。

Core AI 管落地执行。它负责模型转换、AOT 编译、specialization、cache、profiling，把模型放到 CPU、GPU、Neural Engine 这些硬件上。端侧推理不能像服务器那样独占机器，它要和相机、键盘、通知、前台 App、后台任务、电池管理、散热策略一起生活。

我的判断是：Apple 想拿回任务路由权。聊天入口只是表层。谁在本地跑，谁上 PCC，哪个 App 能执行，哪些个人上下文可以被读，哪些结果要回到系统 UI，这些决定如果散在各个 App 里，Apple Intelligence 就不可能成为系统能力。

苹果擅长的就是这种活。模型只是其中一层，系统把模型、App、runtime、隐私、云端路由一起管起来。
...
这时再看 AI PC，就不该只盯着一颗 NPU 或一个 TOPS 数字。至少四层东西要同时摆齐：
...
WWDC26 至少把路线摊开了：自然语言入口、端侧模型、App 能力图谱、PCC、Core AI runtime、Apple Silicon，都放进同一个系统账本。

这条路不一定快。App Intents 需要开发者配合，PCC 需要证明体验和可用性，AFM 3 Core Advanced 的完整技术报告也还没出来。Siri 要从“能听见”变成“能做完”，中间还有很多坑。

一旦 iPhone 能把这条链路跑通，AI PC 的判断也就清楚了。未来本地 AI 的竞争，先看谁能在有限 DRAM、有限功耗、有限散热里留住更多 token，再看谁能把这些 token 变成系统动作。
