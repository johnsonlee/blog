...
把发布内容拆开一层，第一反应会开始松动。Siri 看起来不只是换了一个更会聊天的后端，它开始碰屏幕内容、个人上下文、App 里的动作。反应会变成：好像有点不一样。

再往下，屏幕内容、个人上下文、App 动作、本地模型、PCC 被串起来，事情开始有点意思。等问题落到 iPhone 本地跑 LLM，就开始硬核了。如果一台手机真能把这层理解和路由留在本地，反应会继续升级：有点牛逼。再把内存账算完，就只剩一句：太牛了。
