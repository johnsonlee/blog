...
这次 Siri 被接进了系统推理链路。它要看屏幕内容，读个人上下文，理解 App 里暴露出来的动作，把一句自然语言变成系统可以执行的步骤。旧 Siri 换个聊天后端，做不到这些。

再往下看，屏幕内容、个人上下文、App 动作、本地模型、PCC、Core AI 被串到一条链路上。iPhone 先在本地理解和路由，复杂任务再交给云端。问题也从“它会不会聊天”变成“手机怎么扛住这层推理”，这就硬了。

一台手机要在本地跑 LLM，不只是算力问题。权重放哪，DRAM 放多少，KV Cache 怎么控，什么时候上云，全是硬账。最后只剩一个问题：它是怎么做到的？
