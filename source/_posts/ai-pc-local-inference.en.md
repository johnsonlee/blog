...
Microsoft wants Windows to become the productivity entry point of the AI era again, so it set a hard floor for Copilot+ PC — 40 TOPS of NPU. That's not a spec sheet; it's Microsoft telling the entire OEM camp that the baseline for the next generation of PC is no longer CPU benchmarks but local inference. NVIDIA thinks that's too lightweight. It wants to pull the heavy work — developers, creators, model debugging, code generation — onto the local device, and turn it into an AI workstation.
...
Microsoft is selling a machine, NVIDIA is selling a node, Apple is selling a web. Different words, same thing underneath — **move part of the inference load back from the cloud to the device.**
...
The real shift is that Apple didn't have every app stuff its own LLM inside. It put model capability into the Foundation Models framework and let apps call it through a system API. That step turns the LLM from "a feature of some app" into "a capability of the operating system."
...
> It's not whether the device can run a model, but whether the operating system can make the model a default capability.
...
That's not enough. The real problem of putting a model on a device isn't how big the model file is, but **how much DRAM the runtime occupies.**
...
The key to Apple's on-device path isn't cramming a 20B dense model in. It's a different memory model:
...
Because swapping experts has a cost, Apple doesn't follow the server's token-level MoE. It does prompt-level routing: first judge what capabilities this prompt needs, pick a set of experts and load them into DRAM, reuse them as much as possible across the whole generation, and adjust periodically only when necessary.
...
Whoever controls the default entry point decides whether a task runs locally or in the cloud, which model to use, which app to call, which context to pull, and which result the user ends up seeing. That's the real shared throughline of AI PC and WWDC — not how many TOPS the NPU has, but who owns the entry point.

And Apple's position here is unusual. It isn't the strongest single model, but it holds Apple Silicon, unified memory, on-device models, system permissions, personal context, App Intents, a cross-device ecosystem and PCC all at once. This whole set is a natural fit for on-device AI, especially the Mac — if the iPhone is the carried entry point of personal AI, the Mac is the productivity entry point of local AI.
...
It isn't. The old PC's core was the browser, Office, local files, keyboard and mouse, CPU performance; the new PC's core is local models, personal context, low-latency inference, multimodal input, agent workflows and cloud collaboration. What came back to life isn't the PC as a box, but personal computing devices regaining an irreplaceable local value.
