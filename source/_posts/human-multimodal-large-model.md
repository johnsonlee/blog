...
每个人出厂时的参数不一样。有人天生工作记忆大——context window 长；有人模式识别能力强——某些 attention head 特别好。这些是硬件层的差异，后天训练能优化但改不了上限。
...
把整条链串起来：演化是最早的训练算法，自然选择以生存率为 loss function，基因是权重的序列化格式，繁殖是 checkpoint，变异是 noise injection，死亡是 pruning。文化传承是 distillation，文字的发明是把权重外部化存储。
